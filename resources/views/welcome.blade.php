            ...
            <div class="col">
                <nav>
                    <div class="nav nav-tabs nav-fill" id="nav-tab" role="tablist">
                        <a class="nav-item nav-link active" id="nav-news-tab" data-toggle="tab" href="#nav-news"
                            role="tab" aria-controls="nav-news" aria-selected="true">Latest News</a>
                        @guest
                            <a class="nav-item nav-link" id="nav-register-tab" data-toggle="tab" href="#nav-register"
                                role="tab" aria-controls="nav-register" aria-selected="false">Registers</a>
                        @endguest
                        <a class="nav-item nav-link" id="nav-user-tab" data-toggle="tab" href="#nav-user" role="tab"
                            aria-controls="nav-user" aria-selected="false">List User</a>
                        @guest
                        ...
                        @auth
                            <a class="nav-item nav-link" id="nav-login-tab" data-toggle="tab" href="#nav-login" role="tab"
                                aria-controls="nav-login" aria-selected="false">Profile</a>
                        @endauth
                    </div>
                </nav>
                <div class="tab-content py-3 px-3 px-sm-0" id="nav-tabContent">
                    <div class="tab-pane fade show active" id="nav-news" role="tabpanel" aria-labelledby="nav-news-tab">
                        <ul class="list-unstyled">
                            @foreach ($data['items'] as $data)
                                <li class="media">
                                    <img src="{{ $data['news_image_full'] }}" class="mr-3"
                                        alt="{{ $data['news_image_caption'] }}" style="width: 200px">
                                    <div class="media-body">
                                        <h5 class="mt-0 mb-1">{{ $data['news_title'] }}</h5>
                                        <small
                                            class="text-muted">{{ Carbon::parse($data['news_publish'])->translatedformat('j F, Y') }}</small>
                                        <p>{{ $data['news_teaser'] }}</p>
                                    </div>
                                </li>
                            @endforeach
                            {{-- <li class="media">
                                <img src="https://www.genpi.co/timthumb.php?src=https://photo.jpnn.com/arsip/watermark/2020/08/13/jerinx-sid-foto-instagramjrx-92.jpg&w=232&h=150&zc=1&q=80"
                                    class="mr-3" alt="title">
                                <div class="media-body">
                    ...
                    <div class="tab-pane fade" id="nav-user" role="tabpanel" aria-labelledby="nav-user-tab">
                        <ul class="list-unstyled">
                            @auth
                                @foreach ($user->whereNotIn('id', [Auth::user()->id]) as $user)
                                    <li class="media">
                                        <img src="https://www.genpi.co/timthumb.php?src=https://demo.joomlashine.com/joomla-templates/jsn_metro/free/images/content/content-html/demo-image1.jpg&w=128&h=128&zc=1&q=80"
                                            class="mr-3" alt="">
                                        <div class="media-body">
                                            <h5 class="mt-0 mb-1"><a href="javascript:;" data-toggle="modal"
                                                    data-target="#profile">{{ $user->name }}</a></h5>
                                            <p>{{ $user->alamat }}</p>
                                            <form method="POST" action="{{ route('user.unfollow', $user->id) }}">
                                                @csrf
                                                @method('POST')
                                                <button type="submit" type="button" data-toggle="tooltip"
                                                    data-placement="top" title="Add"
                                                    class="btn btn-primary">Unfollow</button>
                                            </form>
                                            <br>
                                            <form method="POST" action="{{ route('user.follow', $user->id) }}">
                                                ...
                                                @method('POST')
                                                <button type="submit" type="button" data-toggle="tooltip"
                                                    data-placement="top" title="Add" class="btn btn-primary">Follow</button>
                                            </form>
                                        </div>
                                    </li>
                                @endforeach
                                ...
                                @foreach ($user as $user)
                                    <li class="media">
                                        ...
                                        <div class="media-body">
                                            ...
                                            <a href="#nav-login" data-toggle="tab" class="btn btn-primary">Follow</a>
                                        </div>
                                    </li>
                                @endforeach
                            @endguest
                        ...
                        <div class="row">
                            <div class="col-sm-6 ofwwwet-3">
                                ...
                                @auth
                                    <p class="text-center">
                                        <!-- comment sudah login -->
                                        Anda sudah login sebagai {{ Auth::user()->name }} <br /> <a class="dropdown-item"
                                            href="{{ route('logout') }}"
                    ...
                    <div class="modal-body">
                        @auth
                            <div class="media">
                                ...
                                <div class="media-body">
                                    <h5 class="mt-0 mb-1"><a href="javascript:;" data-toggle="modal"
                                            data-target="#profile"></a>{{ $data->name }}</h5>
                                    <p>{{ $user->alamat }}</p>
                                </div>
                            </div>
                        @endauth
